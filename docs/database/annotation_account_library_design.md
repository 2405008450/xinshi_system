...
2. 一条账号记录直接承载一组明文登录账号和密码，备用账号通过 `parent_account_id` 指向主账号。
...
6. 密码更新不覆盖历史，旧明文转入密码历史表。
7. 当前业务约定账号列表直接返回明文凭据；数据库访问、备份和应用权限仍需限制在业务人员范围内。
8. 账号动态字段为全局定义；项目账号字段保存到账号分配履历，按项目隔离；试标和正式安排字段继续按项目隔离。
...
`UNIQUE(platform_id, login_account_normalized)` 是平台级查重的最终数据库防线。当前普通账号列表按业务约定直接返回登录账号和密码明文。

### 4.3 `annotation_account_assignment`

记录账号由谁、在哪个项目、从何时开始使用以及何时释放。`person_id` 可空，用于账号已确定项目和适用语言、但尚未分配人员的场景；此时账号状态仍为 `available`。`custom_values` 保存当前项目账号表的扩展列，账号释放后仍随履历保留。部分唯一索引 `uq_annotation_assignment_active` 约束每个账号最多一条 `released_on IS NULL` 的项目上下文或人员分配。释放日期不得早于分配日期，释放原因限定为项目完成、人员离职、账号封禁、重新分配和其他。
...
- `annotation_accounts:reveal`：单条或批量查看账号明文凭据；每次成功读取均写访问审计。
...
2. 普通列表接口直接返回登录账号和密码明文，账号资产与项目账号表均直接展示。
3. 原显式凭据读取接口保留兼容，调用时仍要求 `annotation_accounts:reveal` 权限并写入访问审计。
