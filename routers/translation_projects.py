...
@router.get("/translator-reconciliation-export")
def export_translator_reconciliation(
    time_field: Literal[
        "customer_reception_time",
        "customer_deadline_time",
        "created_at",
    ...
    try:
        content = create_translation_translator_reconciliation_export(
            db,
            keyword=keyword,
            field_filters=filters,
            sort=sort,
    ...
    time_label = TIME_FIELD_LABELS[time_field]
    filename = f"笔译项目译员对账单_{time_label}_{date_start.isoformat()}_至_{date_end.isoformat()}.xlsx"
    ascii_filename = (
        f"translation-translator-reconciliation-"
        f"{date_start.isoformat()}-{date_end.isoformat()}.xlsx"
    )
    return StreamingResponse(
        BytesIO(content),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
