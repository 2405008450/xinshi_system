...
class ProjectWorkbenchResponsibility(Base):
    ...
    __table_args__ = (
        ...
        Index(
            'uq_workbench_resp_annotation_single_role',
            'annotation_project_id', 'role_code',
            unique=True,
            postgresql_where=text("annotation_project_id IS NOT NULL AND role_code <> 'project_manager'"),
            sqlite_where=text("annotation_project_id IS NOT NULL AND role_code <> 'project_manager'"),
        ),
        Index(
            'uq_workbench_resp_annotation_manager',
            'annotation_project_id', 'role_code', 'assignee_id',
            unique=True,
            postgresql_where=text("annotation_project_id IS NOT NULL AND role_code = 'project_manager' AND assignee_id IS NOT NULL"),
            sqlite_where=text("annotation_project_id IS NOT NULL AND role_code = 'project_manager' AND assignee_id IS NOT NULL"),
        ),
