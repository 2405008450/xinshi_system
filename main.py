...
def ensure_customer_charge_item_columns():
    """兼容本地历史库：将客户收费项扩展到母订单并补齐对账字段。"""
    inspector = inspect(engine)
    if "translation_sub_order_charge_item" not in inspector.get_table_names():
        return
    with engine.begin() as conn:
        conn.execute(text("""
        ...
        conn.execute(text("""
            ...
            WHERE unit_price_excl_tax IS NULL AND unit_price IS NOT NULL
        """))
        conn.execute(text("""
            UPDATE translation_sub_order_charge_item
            SET total_excl_tax = amount_override
            WHERE total_excl_tax IS NULL AND amount_override IS NOT NULL
        """))
        conn.execute(text("""
            UPDATE translation_sub_order_charge_item item
            SET total_excl_tax = ROUND(
                metric.count_value::NUMERIC / item.unit_size * item.unit_price, 2
            )
        ...
        conn.execute(text("""
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_sub_order_charge_item_project') THEN
                    ALTER TABLE translation_sub_order_charge_item
                ...
                END IF;
                IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_sub_order_charge_item_single_owner') THEN
                    ALTER TABLE translation_sub_order_charge_item
                        ADD CONSTRAINT ck_sub_order_charge_item_single_owner
                        CHECK (num_nonnulls(translation_project_id, sub_order_id) = 1);
                END IF;
                IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_sub_order_charge_item_billing_month') THEN
                    ALTER TABLE translation_sub_order_charge_item
                        ADD CONSTRAINT ck_sub_order_charge_item_billing_month
                        CHECK (billing_month IS NULL OR billing_month ~ '^[0-9]{4}-(0[1-9]|1[0-2])$');
                END IF;
                IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_customer_charge_item_prices_nonnegative') THEN
                    ALTER TABLE translation_sub_order_charge_item
...
def run_runtime_migrations():
    ...
    ensure_customer_charge_item_columns()
