<template>
  <el-card class="page-card">
    <template #header>
      <div class="header">
        ...
        <div class="actions">
          ...
          <CustomFieldManager table-code="account" @changed="loadFields" />
          ...
          <el-tooltip v-if="!editing && !deleteMode" :disabled="Boolean(effectiveClientId)" content="所选项目尚未关联客户，请先在项目详情中完善客户信息" placement="bottom">
            <span class="disabled-button-wrapper"><el-button :disabled="!effectiveClientId" @click="enterCreateMode">新增账号</el-button></span>
          </el-tooltip>
    ...
    <div class="filters">
      <el-select v-model="clientId" :disabled="editing" filterable remote clearable :loading="clientSearchLoading" :remote-method="searchClients" placeholder="输入客户名称后联想" style="width:260px" @change="changeClient">
        <el-option v-for="item in clients" :key="item.id" :label="item.client_short_name || item.client_name" :value="item.id" />
      </el-select>
      <el-select v-model="projectId" :disabled="editing" clearable filterable placeholder="分配项目（选择人员前必选）" style="width:min(420px, calc(100vw - 32px))" @change="projectSelectionChanged">
        <el-option v-for="item in clientProjects" :key="item.id" :label="item.projectName || '未命名'" :value="item.id" />
      </el-select>
      <div class="assignment-language-field">
        <el-select v-model="assignmentLanguageItemId" :disabled="editing||!projectId" clearable filterable placeholder="账号适用语言（选择人员前必选）" style="width:250px">
      ...
      <el-input v-model="keyword" :disabled="editing" clearable placeholder="平台、编号、姓名或昵称" style="width:280px" @input="onKeyword" @keyup.enter="queryNow" />
      <el-button type="primary" :disabled="editing" @click="queryNow">查询</el-button><el-button :disabled="editing" @click="resetFilters">重置</el-button>
      <el-popover v-model:visible="advancedVisible" trigger="click" placement="bottom-end" :width="760" popper-class="account-advanced-popper">
        <template #reference><el-button :disabled="editing">高级筛选{{ advancedCount ? `（${advancedCount}）` : '' }}</el-button></template>
    ...
    <div v-if="editing" class="sheet-toolbar">
      ...
      <el-checkbox v-model="passwordVisible">密码明文</el-checkbox>
    ...
    <div v-if="editing" class="batch-defaults">
      ...
      <el-form label-position="top" class="batch-defaults-form">
        ...
        <el-form-item label="项目"><el-select v-model="batchDefaults.projectId" clearable filterable @change="batchProjectChanged"><el-option v-for="item in clientProjects" :key="item.id" :label="item.projectName || '未命名'" :value="item.id" /></el-select></el-form-item>
        ...
        <el-form-item label="账号来源"><el-select v-model="batchDefaults.accountSource" @change="syncBatchDefaults('accountSource')"><el-option v-for="(label,value) in sourceLabels" :key="value" :label="label" :value="value" /></el-select></el-form-item>
        <el-form-item label="负责人"><el-select v-model="batchDefaults.ownerId" filterable @change="syncBatchDefaults('ownerId')"><el-option v-for="item in users" :key="item.id" :label="userName(item)" :value="item.id" /></el-select></el-form-item>
        <el-form-item label="到期日"><el-date-picker v-model="batchDefaults.expiresOn" clearable value-format="YYYY-MM-DD" @change="syncBatchDefaults('expiresOn')" /></el-form-item>
      </el-form>
    </div>
    ...
    <div v-if="editing" class="sheet-wrap" @paste="handleGridPaste">
      <el-table ref="editTableRef" :data="editRows" border row-key="_rowKey" :row-class-name="draftRowClass" @selection-change="draftSelectionChanged">
        ...
        <el-table-column label="账号昵称" min-width="180"><template #default="{row,$index}"><div :data-cell="`${$index}:nickname`"><el-input v-model="row.nickname" @focus="setActiveCell($index,'nickname')" @input="markDraft(row)" @keydown="gridKeydown($event,$index,'nickname')" /></div></template></el-table-column>
        ...
        <el-table-column label="密码" min-width="220"><template #default="{row,$index}"><div :data-cell="`${$index}:password`"><el-input v-model="row.password" :type="passwordVisible?'text':'password'" autocomplete="new-password" @focus="setActiveCell($index,'password')" @input="markDraft(row)" @keydown="gridKeydown($event,$index,'password')" /></div></template></el-table-column>
        <el-table-column label="项目" min-width="200"><template #default="{row}"><el-select v-model="row.projectId" clearable filterable @change="draftProjectChanged(row)"><el-option v-for="item in clientProjects" :key="item.id" :label="item.projectName || item.orderNo || '未命名'" :value="item.id" /></el-select></template></el-table-column>
        ...
        <el-table-column label="账号状态" min-width="130"><template #default="{row}"><el-select v-model="row.accountStatus" :disabled="Boolean(row.personId)" @change="markDraft(row)"><el-option v-for="(label,value) in row.personId?{assigned:'已分配'}:manualAccountStatusLabels" :key="value" :label="label" :value="value" /></el-select></template></el-table-column>
        <el-table-column label="注册状态" min-width="150"><template #default="{row}"><el-select v-model="row.registrationStatus" @change="markDraft(row)"><el-option v-for="(label,value) in registrationStatusLabels" :key="value" :label="label" :value="value" /></el-select></template></el-table-column>
        <el-table-column label="账号来源" min-width="150"><template #default="{row}"><el-select v-model="row.accountSource" @change="markDraft(row)"><el-option v-for="(label,value) in sourceLabels" :key="value" :label="label" :value="value" /></el-select></template></el-table-column>
        <el-table-column label="负责人*" min-width="160"><template #default="{row}"><el-select v-model="row.ownerId" filterable @change="markDraft(row)"><el-option v-for="item in users" :key="item.id" :label="userName(item)" :value="item.id" /></el-select></template></el-table-column>
        <el-table-column label="到期日" min-width="160"><template #default="{row}"><el-date-picker v-model="row.expiresOn" clearable value-format="YYYY-MM-DD" @change="markDraft(row)" /></template></el-table-column>
        <el-table-column label="备注" min-width="220"><template #default="{row}"><el-input v-model="row.remarks" type="textarea" :rows="1" @input="markDraft(row)" /></template></el-table-column>
        <el-table-column v-for="field in customFields" :key="field.id" :label="field.fieldLabel" min-width="180">
    ...
    <el-table ref="accountTableRef" v-else :data="rows" v-loading="loading" border row-key="id" @selection-change="handleDeleteSelectionChange">
      ...
      <el-table-column v-if="isVisible('platform')" label="平台链接" min-width="220"><template #default="{row}"><el-link class="platform-link" type="primary" :href="row.platformUrl" target="_blank" rel="noopener noreferrer" :title="row.platformUrl">{{ row.platformUrl || '-' }}</el-link></template></el-table-column>
      <el-table-column v-if="isVisible('nickname')" prop="nickname" label="账号昵称" min-width="130"><template #default="{row}">{{ row.nickname || '-' }}</template></el-table-column>
      <el-table-column prop="loginAccount" label="登录账号" min-width="170"><template #default="{row}">{{ row.loginAccount || '-' }}</template></el-table-column>
      <el-table-column prop="password" label="密码" min-width="160"><template #default="{row}">{{ row.password || '-' }}</template></el-table-column>
      <el-table-column v-if="isVisible('person')" label="标注员" min-width="220"><template #default="{row}">
        <el-select
      ...
      <el-table-column v-if="isVisible('project')" prop="projectName" label="当前项目" min-width="160"><template #default="{row}">{{ row.projectName || '-' }}</template></el-table-column>
      ...
      <el-table-column v-if="isVisible('accountStatus')" label="账号状态" width="100"><template #default="{row}"><el-tag size="small" :type="statusTag(row.accountStatus)">{{ accountStatusLabels[row.accountStatus] || row.accountStatus }}</el-tag></template></el-table-column>
      <el-table-column v-if="isVisible('owner')" label="负责人" min-width="120"><template #default="{row}">{{ row.ownerName || '-' }}</template></el-table-column>
      <el-table-column v-if="isVisible('expiresOn')" prop="expiresOn" label="到期日" width="120"><template #default="{row}">{{ row.expiresOn || '-' }}</template></el-table-column>
      <el-table-column v-for="field in visibleCustomFields" :key="field.id" :label="field.fieldLabel" min-width="120" show-overflow-tooltip><template #default="{row}">{{ formatValue(row.customValues?.[field.id]) }}</template></el-table-column>
      ...
      <el-table-column v-if="!deleteMode" label="操作" width="90" fixed="right"><template #default="{row}"><el-button link type="primary" @click="openAccount(row)">编辑</el-button></template></el-table-column>
    </el-table>
    <div v-if="!editing" class="pagination"><el-pagination v-model:current-page="pagination.page" v-model:page-size="pagination.limit" :total="pagination.total" :page-sizes="[20,50,100]" layout="total, sizes, prev, pager, next, jumper" @current-change="reload" @size-change="pageSizeChanged" /></div>
  </el-card>
  ...
  <el-dialog v-model="accountDialog" title="编辑账号" width="min(880px, calc(100vw - 32px))" top="5vh" class="long-dialog" append-to-body>
    <el-form label-width="110px"><el-row :gutter="16"><el-col :xs="24" :md="12"><el-form-item label="平台" required><el-select v-model="accountForm.platformId" style="width:100%"><el-option v-for="item in platforms" :key="item.id" :label="platformName(item)" :value="item.id" /></el-select></el-form-item></el-col><el-col :xs="24" :md="12"><el-form-item label="账号昵称"><el-input v-model="accountForm.nickname" /></el-form-item></el-col>
      <el-col :xs="24" :md="12"><el-form-item label="登录账号"><el-input v-model="accountForm.loginAccount" placeholder="留空表示不修改" autocomplete="off" /></el-form-item></el-col><el-col :xs="24" :md="12"><el-form-item label="密码"><el-input v-model="accountForm.password" type="password" show-password placeholder="留空表示不修改" autocomplete="new-password" /></el-form-item></el-col>
...
const clients=ref([]),projects=ref([]),platforms=ref([]),rows=ref([]),talents=ref([]),users=ref([]),customFields=ref([])
...
const editing=ref(false),editRows=ref([]),selectedDrafts=ref([]),editTableRef=ref(null),accountTableRef=ref(null),passwordVisible=ref(true)
...
const advancedVisible=ref(false),platformManagerVisible=ref(false),platformDialog=ref(false),accountDialog=ref(false)
...
const tableColumns=computed(()=>[{key:'platform',label:'平台链接'},{key:'nickname',label:'账号昵称'},{key:'person',label:'标注员'},{key:'project',label:'当前项目'},{key:'language',label:'语言方向'},{key:'accountStatus',label:'账号状态'},{key:'owner',label:'负责人'},{key:'expiresOn',label:'到期日'},...customFields.value.map(item=>({key:`custom:${item.id}`,label:item.fieldLabel}))])
const {selectedKeys:visibleColumnKeys,isVisible,reset:resetColumns}=useTableColumns('annotation-account-library-v3',tableColumns,['platform','nickname','person','language','accountStatus','owner'])
...
const reload=async()=>{clearTimeout(timer);controller?.abort();controller=new AbortController();const current=++requestId;loading.value=true;const query=buildFilters();try{const [data,count]=await Promise.all([ops.getAccounts({skip:(pagination.page-1)*pagination.limit,limit:pagination.limit,...query},{signal:controller.signal}),ops.getAccountCount(query,{signal:controller.signal})]);if(current!==requestId)return;rows.value=Array.isArray(data)?data:[];pagination.total=count?.total||0;await loadOccupancy()}catch(error){if(current!==requestId||error?.code==='ERR_CANCELED')return;ElMessage.error(error.detail||'加载账号失败')}finally{if(current===requestId)loading.value=false}}
...
const projectSelectionChanged=async()=>{filters.languageItemId='';syncAssignmentLanguage();pagination.page=1;await loadPlatforms();await reload()}
const clearAdvanced=()=>{Object.assign(filters,{platformId:'',assignmentState:'',accountStatus:'',languageItemId:''});queryNow()}
const changeClient=async()=>{projectId.value='';assignmentLanguageItemId.value='';Object.assign(filters,{platformId:'',assignmentState:'',accountStatus:'',languageItemId:''});pagination.page=1;await loadPlatforms();await reload()}
const resetFilters=async()=>{clientId.value='';clients.value=[];projectId.value='';assignmentLanguageItemId.value='';keyword.value='';Object.assign(filters,{platformId:'',assignmentState:'',accountStatus:'',languageItemId:''});await loadPlatforms();queryNow()}
...
const accountBatchPayload=(row,personId,context)=>({
  ...
  rows:[{
    ...
    projectId:personId?context.projectId:null,
    languageItemIds:personId?context.languageItemIds:[],
  }],
})
...
const discardDrafts=async()=>{
  if(dirtyCount.value){
    try{await ElMessageBox.confirm(`还有 ${dirtyCount.value} 行未保存，确定放弃这些修改？`,'未保存修改',{type:'warning',confirmButtonText:'放弃修改',cancelButtonText:'继续编辑'})}catch{return false}
  }
  return true
}
...
const saveDrafts=async()=>{
  ...
  try{
    const response=await ops.batchSaveAccounts({clientId:effectiveClientId.value,rows:drafts.map(row=>({
      ...
      personId:row.personId||null,projectId:row.projectId||null,languageItemIds:row.languageItemIds||[],
    }))})
...
const openAccount=async row=>{
  ...
  Object.assign(accountForm,{id:row.id,clientId:row.clientId||'',platformId:row.platformId,parentAccountId:row.parentAccountId||null,ownerId:row.ownerId||currentUserId,nickname:row.nickname||'',loginAccount:'',password:'',accountStatus:row.accountStatus||'available',registrationStatus:row.registrationStatus||'unregistered',accountSource:row.accountSource||'client_provided',expiresOn:row.expiresOn||null,remarks:row.remarks||'',customValues:{...(row.customValues||{})},personId:row.personId||'',originalPersonId:row.personId||'',projectId:resolvedProjectId,originalProjectId:row.projectId||'',languageItemIds:resolvedLanguages})
...
const saveAccount=async()=>{
  ...
  saving.value=true
  try{
    const {clientId:formClientId,personId,originalPersonId,projectId:formProjectId,originalProjectId,languageItemIds,...formAccount}=accountForm
    ...
    if(usesBatchSave){
      const response=await ops.batchSaveAccounts({clientId:formClientId||effectiveClientId.value,rows:[{rowKey:accountForm.id,id:accountForm.id,account,personId:personId||null,projectId:formProjectId||null,languageItemIds}]})
      const result=response.results?.[0]
...
@media(max-width:1200px){.batch-defaults-form{grid-template-columns:repeat(3,minmax(160px,1fr))}}@media(max-width:768px){.header{align-items:flex-start;flex-direction:column;gap:12px}.batch-defaults-form{grid-template-columns:1fr}.pagination{overflow-x:auto;justify-content:flex-start}}
</style>
