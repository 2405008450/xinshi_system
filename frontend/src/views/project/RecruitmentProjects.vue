  ...
  <el-card class="compact-list-card">
    ...
    <el-table ref="projectTableRef" :data="rows" v-loading="loading" :row-class-name="projectRowClass" border row-key="id" class="recruitment-list-table project-detail-list-table" @selection-change="handleDeleteSelectionChange">
      ...
      <el-table-column v-for="column in visibleColumns" :key="column.key" :prop="column.key" :label="column.label" :width="column.width" :min-width="column.minWidth" :show-overflow-tooltip="column.key !== 'projectName' && column.tooltip !== false">
        ...
        <template #default="{ row }">
          ...
          <el-button v-else-if="column.key === 'projectName'" type="primary" link class="project-name-ellipsis business-clickable-cell" :title="row.projectName || '待生成'" @click="openProgress(row)">{{ row.projectName || '待生成' }}</el-button>
      ...
      <div ref="editorBodyRef" class="editor-body">
        <AppForm ref="formRef" :model="form" :rules="rules" label-width="140px" class="recruitment-form">
          ...
          <div class="form-section"><h3>服务费用与扩展信息</h3>
            <el-row :gutter="16">
              <el-col :xs="24" :md="8"><el-form-item label="结算方式"><el-select v-model="form.serviceFeeType" clearable placeholder="选择结算方式" style="width:100%"><el-option label="固定金额" value="fixed" /><el-option label="年薪比例" value="annual_salary_rate" /><el-option label="月薪倍数" value="monthly_salary_multiple" /><el-option label="其他" value="other" /></el-select></el-form-item></el-col>
              ...
              <el-col v-if="form.serviceFeeType==='monthly_salary_multiple'" :xs="24" :md="16"><el-form-item label="月薪倍数" prop="serviceFeeMultiplier"><el-input-number v-model="form.serviceFeeMultiplier" :min="0" :precision="2" :controls="false" placeholder="请输入倍数" /><span class="suffix">倍</span></el-form-item></el-col>
            </el-row>
            <el-form-item label="费用说明"><el-input v-model="form.serviceFeeNote" /></el-form-item>
            <el-row :gutter="16"><el-col :xs="24" :md="12"><el-form-item label="客户咨询时间"><el-date-picker v-model="form.customerConsultationTime" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" style="width:100%" format="YYYY-MM-DD HH:mm" time-format="HH:mm" :show-now="true" :show-confirm="true" :show-footer="true" /></el-form-item></el-col><el-col :xs="24" :md="12"><el-form-item label="客户确认时间"><el-date-picker v-model="form.customerConfirmationTime" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" style="width:100%" format="YYYY-MM-DD HH:mm" time-format="HH:mm" :show-now="true" :show-confirm="true" :show-footer="true" /></el-form-item></el-col></el-row>
    ...
    <DraggableFormDialog v-model="progressVisible" width="min(760px, calc(100vw - 32px))" top="5vh" class="progress-dialog">
      ...
      <div class="inline-create progress-create" v-if="canWrite">
        <el-date-picker v-model="progressOccurredAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" placeholder="选择发生时间" format="YYYY-MM-DD HH:mm" time-format="HH:mm" :show-now="true" :show-confirm="true" :show-footer="true" />
    ...
    <DraggableFormDialog v-model="candidateEditorVisible" :title="candidateForm.id ? '编辑候选人' : '新增候选人'" width="min(760px, calc(100vw - 32px))" top="5vh" append-to-body class="candidate-editor-dialog">
      <div class="candidate-editor-body"><AppForm ref="candidateFormRef" :model="candidateForm" :rules="candidateRules" label-width="110px">
        ...
        <el-collapse><el-collapse-item title="更多信息" name="legacy">
          ...
          <el-row :gutter="16"><el-col :xs="24" :md="12"><el-form-item label="推荐时间"><el-date-picker v-model="candidateForm.recommendedAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" style="width:100%" format="YYYY-MM-DD HH:mm" time-format="HH:mm" :show-now="true" :show-confirm="true" :show-footer="true" /></el-form-item></el-col><el-col :xs="24" :md="12"><el-form-item label="原面试时间"><el-date-picker v-model="candidateForm.interviewAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" style="width:100%" format="YYYY-MM-DD HH:mm" time-format="HH:mm" :show-now="true" :show-confirm="true" :show-footer="true" /></el-form-item></el-col></el-row>
          <el-row :gutter="16"><el-col :xs="24" :md="12"><el-form-item label="Offer时间"><el-date-picker v-model="candidateForm.offerAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" style="width:100%" format="YYYY-MM-DD HH:mm" time-format="HH:mm" :show-now="true" :show-confirm="true" :show-footer="true" /></el-form-item></el-col><el-col :xs="24" :md="12"><el-form-item label="下次跟进"><el-date-picker v-model="candidateForm.nextFollowUpAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" style="width:100%" format="YYYY-MM-DD HH:mm" time-format="HH:mm" :show-now="true" :show-confirm="true" :show-footer="true" /></el-form-item></el-col></el-row>
...
const statusOptions = [
  ['pending_setup','新建待立项'],['sourcing','立项启动（寻访阶段）'],['recommending','简历推荐中'],['interviewing','面试进行中'],['offer_negotiation','Offer谈判阶段'],['pending_onboard','候选人待入职'],['probation','已入职保用期'],['full_time_dispatch','全职外派'],['cancelled','项目取消'],['closed','项目结案'],
...
const statusType = (value) => ({ pending_setup:'info', sourcing:'primary', recommending:'warning', interviewing:'warning', offer_negotiation:'warning', pending_onboard:'primary', probation:'success', full_time_dispatch:'success', cancelled:'danger', closed:'success' }[value] || 'info')
...
const loadReferenceData=async()=>{const results=await Promise.allSettled([getUsers({skip:0,limit:500}),getClients({skip:0,limit:500}),getRecruitmentResumeSources(),getRecruitmentTalents({skip:0,limit:500}),getProjectLanguages()]);users.value=results[0].status==='fulfilled'?(results[0].value||[]):[];clients.value=results[1].status==='fulfilled'?(results[1].value||[]):[];resumeSources.value=results[2].status==='fulfilled'?(results[2].value||[]):[];talentOptions.value=results[3].status==='fulfilled'?(results[3].value||[]):[];languages.value=results[4].status==='fulfilled'?(results[4].value||[]):[]}
const projectRowClass=({row})=>String(row.id)===highlightedProjectId.value?'workbench-target-row':''
const focusRouteProject=async(editorReady=Promise.resolve())=>{const projectId=String(route.query.projectId||'');if(!projectId)return;try{const detail=await getRecruitmentProject(projectId);highlightedProjectId.value=projectId;searchForm.keyword=detail.orderNo||'';pagination.page=1;const listPromise=fetchData();if(route.query.openEditor==='1'){await editorReady;await openEdit(detail,true);const query={...route.query};delete query.openEditor;await router.replace({query})}await Promise.all([listPromise,editorReady])}catch(error){ElMessage.error(error?.detail||'定位招聘项目失败')}}
...
const rules = { projectStatus:[{required:true,message:'请选择项目状态',trigger:'change'}], positionTitle:[{required:true,message:'请输入职位名称/类型',trigger:'blur'}], headcountMin:[{required:true,message:'请输入招聘人数',trigger:'change'}], employmentRange:[{required:true,message:'请选择拟履职周期',trigger:'change'}], workLocation:[{required:true,message:'请输入任职工作属地',trigger:'blur'}], serviceFeeAmount:[{trigger:'change',validator:(_rule,value,callback)=>form.serviceFeeType==='fixed'&&value==null?callback(new Error('固定金额服务费必须填写金额')):callback()}], serviceFeeRate:[{trigger:'change',validator:(_rule,value,callback)=>form.serviceFeeType==='annual_salary_rate'&&value==null?callback(new Error('年薪比例服务费必须填写比例')):callback()}], serviceFeeMultiplier:[{trigger:'change',validator:(_rule,value,callback)=>form.serviceFeeType==='monthly_salary_multiple'&&value==null?callback(new Error('月薪倍数服务费必须填写倍数')):callback()}] }
...
const previewProjectName = async ({ automatic = false } = {}) => {
  ...
  try {
    ...
    if (!automatic) ElMessage.error(error?.detail || '项目名称生成失败')
  } finally {
