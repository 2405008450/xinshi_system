  ...
  <div class="annotation-workflow-page">
    <el-card class="workflow-card compact-list-card">
      ...
      <el-table ref="workflowTableRef" :data="rows" v-loading="loading" border row-key="id" @selection-change="handleDeleteSelectionChange">
        ...
        <el-table-column label="详情" width="90" fixed="right" align="center"><template #default="{row}"><BusinessDetailPopover :row="row" title="正式标注安排详情"><template #content><el-descriptions :column="2" border size="small"><el-descriptions-item label="订单号">{{ row.projectOrderNo||'-' }}</el-descriptions-item><el-descriptions-item label="项目名称">{{ row.projectName||'-' }}</el-descriptions-item><el-descriptions-item label="客户简称">{{ row.clientShortName||'-' }}</el-descriptions-item><el-descriptions-item label="角色">{{ roleLabels[row.assignmentRole]||'-' }}</el-descriptions-item><el-descriptions-item label="人员">{{ row.personName||'-' }}（{{ row.resourceCode||'-' }}）</el-descriptions-item><el-descriptions-item label="进行状态">{{ assignmentStatusLabels[row.assignmentStatus]||'-' }}</el-descriptions-item><el-descriptions-item label="语种">{{ row.languageDisplay||'-' }}</el-descriptions-item><el-descriptions-item label="音频长度">{{ amountText(row.audioDurationValue,row.audioDurationUnit,durationUnitLabels) }}</el-descriptions-item><el-descriptions-item label="人员价格" :span="2">{{ priceText(row.amount,row.unit,row.currency) }}</el-descriptions-item><el-descriptions-item label="质量评分">{{ row.qualityScore||'-' }}</el-descriptions-item><el-descriptions-item label="评价备注" :span="2">{{ row.evaluationNote||'-' }}</el-descriptions-item><el-descriptions-item v-for="field in customFields" :key="field.id" :label="field.fieldLabel" :span="field.dataType==='textarea'?2:1">{{ customText(row.customValues?.[field.id]) }}</el-descriptions-item></el-descriptions></template></BusinessDetailPopover></template></el-table-column>
        <el-table-column v-if="canWrite && !deleteMode" label="操作" width="120" fixed="right" align="center"><template #default="{row}"><PrimaryEditButton @click="openEditor(row)" /></template></el-table-column>
      </el-table>
