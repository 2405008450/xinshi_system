<template>
  <el-card><template #header><div class="header"><div><h2>试标流程</h2><p>默认展示全部项目的试标进度；选择项目后仅查看该项目</p></div><div class="actions"><CustomFieldManager v-if="projectId" table-code="trial" :project-id="projectId" @changed="loadFields" /><el-button type="primary" @click="openEditor()">新增试标</el-button></div></div></template>
    ...
    <el-table :data="displayRows" v-loading="loading" border><el-table-column type="index" label="序号" width="65" /><el-table-column prop="projectOrderNo" label="订单号" width="130" /><el-table-column prop="projectName" label="项目名称" min-width="180" show-overflow-tooltip /><el-table-column prop="clientShortName" label="客户简称" width="120"><template #default="{row}">{{ row.clientShortName||'-' }}</template></el-table-column><el-table-column prop="resourceCode" label="标注员编号" width="120" /><el-table-column prop="personName" label="姓名" width="120" /><el-table-column prop="roundNo" label="轮次" width="75" /><el-table-column prop="willingnessText" label="意愿" min-width="150" show-overflow-tooltip /><el-table-column label="状态" width="100"><template #default="{row}"><el-tag :type="statusType(row.trialStatus)" size="small">{{ statusLabels[row.trialStatus] }}</el-tag></template></el-table-column><el-table-column label="结果" width="110"><template #default="{row}">{{ resultLabels[row.trialResult]||'-' }}</template></el-table-column>
      ...
      <el-table-column label="详情" width="90" fixed="right"><template #default="{row}"><el-popover trigger="click" placement="left" :width="760" title="试标详情"><template #reference><el-button link type="primary">查看详情</el-button></template><div class="detail"><el-descriptions :column="2" border size="small"><el-descriptions-item label="订单号">{{ row.projectOrderNo||'-' }}</el-descriptions-item><el-descriptions-item label="项目名称">{{ row.projectName||'-' }}</el-descriptions-item><el-descriptions-item label="客户简称">{{ row.clientShortName||'-' }}</el-descriptions-item><el-descriptions-item label="标注员">{{ row.personName }}（{{ row.resourceCode||'-' }}）</el-descriptions-item><el-descriptions-item label="轮次">第 {{ row.roundNo }} 轮</el-descriptions-item><el-descriptions-item label="状态">{{ statusLabels[row.trialStatus] }}</el-descriptions-item><el-descriptions-item label="意愿" :span="2">{{ row.willingnessText||'-' }}</el-descriptions-item><el-descriptions-item label="结果">{{ resultLabels[row.trialResult]||'-' }}</el-descriptions-item><el-descriptions-item label="评语" :span="2">{{ row.resultNote||'-' }}</el-descriptions-item></el-descriptions></div></el-popover></template></el-table-column>
      <el-table-column label="操作" width="120" fixed="right"><template #default="{row}"><el-button link type="primary" @click="openEditor(row)">编辑</el-button><el-button link type="danger" @click="remove(row)">删除</el-button></template></el-table-column></el-table>
  </el-card>
...
const projects=ref([]),talents=ref([]),rows=ref([]),customFields=ref([]),editorFields=ref([]),accountOptions=ref([]),projectId=ref(''),keyword=ref(''),statusFilter=ref(''),loading=ref(false),accountLoading=ref(false),dialogVisible=ref(false),saving=ref(false)
...
const customText=(value)=>Array.isArray(value)?value.join('、'):value??'-'
...
const remove=async(row)=>{await ElMessageBox.confirm(`删除 ${row.personName} 的第 ${row.roundNo} 轮试标？`,'确认删除');await ops.deleteTrial(row.id);await search()}
...
<style scoped>.header,.actions,.filters{display:flex;align-items:center}.header{justify-content:space-between}.header h2{margin:0}.header p{margin:4px 0 0;color:var(--el-text-color-secondary)}.actions,.filters{gap:8px}.filters{margin-bottom:16px;flex-wrap:wrap}.detail{max-height:560px;overflow:auto}</style>
