<template>
  <el-card class="compact-list-card"><template #header><div class="card-header"><div><h2>试标流程</h2><p>默认展示全部项目的试标进度；选择项目后仅查看该项目</p></div><div class="header-actions"><CustomFieldManager v-if="canWrite && projectId" table-code="trial" :project-id="projectId" @changed="loadFields" /><BatchDeleteToolbar v-if="canWrite" :active="deleteMode" :selected-count="selectedRows.length" :loading="deleting" @enter="enterDeleteMode" @exit="exitDeleteMode" @confirm="confirmBatchDelete" /><el-button v-if="canWrite && !deleteMode" type="primary" @click="openEditor()">新增试标</el-button></div></div></template>
    <div class="filters"><el-select v-model="projectId" clearable filterable placeholder="全部标注项目" style="width:min(360px, calc(100vw - 32px))" @change="changeProject"><el-option v-for="item in projects" :key="item.id" :label="`${item.orderNo || '-'} · ${item.projectName || '未命名'}`" :value="item.id" /></el-select><el-select v-model="personId" clearable filterable placeholder="全部标注员" style="width:220px" @change="search"><el-option v-for="item in talents" :key="item.id" :label="`${item.fullName}（${item.resourceCode||'-'}）`" :value="item.id" /></el-select><el-input v-model="keyword" clearable placeholder="搜索项目、编号、姓名、意愿或评语" style="width:280px" @input="onInput" @keyup.enter="search" /><el-select v-model="statusFilter" clearable placeholder="试标状态" @change="search"><el-option v-for="(label,value) in statusLabels" :key="value" :label="label" :value="value" /></el-select><el-button type="primary" @click="search">查询</el-button><el-button @click="reset">重置</el-button></div>
...
const projects=ref([]),talents=ref([]),rows=ref([]),customFields=ref([]),editorFields=ref([]),accountOptions=ref([]),projectId=ref(''),personId=ref(''),keyword=ref(''),statusFilter=ref(''),loading=ref(false),accountLoading=ref(false),dialogVisible=ref(false),saving=ref(false),trialTableRef=ref(null)
...
const search=async()=>{clearTimeout(timer);controller?.abort();controller=new AbortController();const current=++requestId;loading.value=true;try{const result=await ops.getTrials(projectId.value,{personId:personId.value||undefined,skip:0,limit:500,keyword:keyword.value.trim()||undefined,trialStatus:statusFilter.value||undefined},{signal:controller.signal});if(current===requestId)rows.value=result;await loadFields()}catch(error){if(error.code!=='ERR_CANCELED')ElMessage.error(error.detail||'加载试标记录失败')}finally{if(current===requestId)loading.value=false}}
...
const reset=()=>{projectId.value='';personId.value='';keyword.value='';statusFilter.value='';customFields.value=[];search()}
...
const applyRouteFilters=()=>{projectId.value=String(route.query.projectId||'');personId.value=String(route.query.personId||'')}
onMounted(async()=>{const [ps,ts]=await Promise.all([projectApi.getAnnotationProjects({skip:0,limit:500}),talentApi.getProjectTalentOptions('annotation')]);projects.value=ps;talents.value=ts;applyRouteFilters();await search()})
watch(()=>[route.query.projectId,route.query.personId],([nextProjectId,nextPersonId],[previousProjectId,previousPersonId])=>{if(nextProjectId===previousProjectId&&nextPersonId===previousPersonId)return;applyRouteFilters();void search()})
