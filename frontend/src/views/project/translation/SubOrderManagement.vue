...
const loadData = async () => { loading.value = true; try { await Promise.all([fetchProject(), fetchSubOrders()]) } catch (error) { ElMessage.error(getLocalizedErrorMessage(error, '加载子订单失败')) } finally { loading.value = false } }
