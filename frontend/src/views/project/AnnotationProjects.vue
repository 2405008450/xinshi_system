      ...
      <div ref="dialogBodyRef" class="editor-body">
        <el-form ref="formRef" :model="form" :rules="rules" label-width="125px">
          <section class="form-section">
            ...
            <el-row :gutter="16">
              <el-col :xs="24" :md="12"><el-form-item label="负责人联系方式"><ReadonlyField :model-value="form.managerContact" :source="form.clientId ? 'auto' : 'editable'" :placeholder="form.clientId ? '选择客户后自动带出' : '新客户可填写负责人联系方式'" @update:model-value="form.managerContact = $event" /></el-form-item></el-col>
            </el-row>
          </section>
...
const focusRouteProject=async()=>{const projectId=String(route.query.projectId||'');if(!projectId)return;const detail=await loadDetail(projectId);if(!detail)return;highlightedProjectId.value=projectId;searchForm.keyword=detail.orderNo||'';pagination.page=1;await fetchData()}
...
onMounted(async()=>{await Promise.all([loadReferenceData(),loadProjectCustomFields()]);await fetchData();await focusRouteProject()})
