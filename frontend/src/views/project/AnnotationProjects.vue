...
const handleChatMessageToProgress=async(message)=>{const content=String(message?.content||'').trim();if(!content)return ElMessage.warning('该消息没有可添加的文字内容');const existing=String(statusForm.changeNote||'').trim();if(existing&&existing!==content){try{await ElMessageBox.confirm('具体进度中已有未保存内容，是否使用所选消息替换？','替换具体进度',{type:'warning',confirmButtonText:'替换',cancelButtonText:'取消'})}catch{return}}const messageDate=new Date(message?.createdAt);const effectiveDate=Number.isNaN(messageDate.getTime())?new Date():messageDate;statusEntryMode.value='progress';selectedProgressStageKey.value='';progressDraftSource.value={senderName:message?.senderName||'未知用户',createdAt:Number.isNaN(messageDate.getTime())?effectiveDate:messageDate};Object.assign(statusForm,{projectStatus:activeProgressProject.value?.projectStatus||'',effectiveOn:localDateTimeValue(effectiveDate),changeNote:content});progressDialogTab.value='progress';await nextTick();statusFormRef.value?.clearValidate();progressEntryPanelRef.value?.scrollIntoView({behavior:'smooth',block:'nearest'});progressNoteInputRef.value?.focus?.();ElMessage.success('消息已带入具体进度，请确认后保存')}
