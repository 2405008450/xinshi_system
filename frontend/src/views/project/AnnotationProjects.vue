<template>
  <el-card class="annotation-card">
    ...
    <el-table :data="tableData" v-loading="loading" border class="annotation-table">
      ...
      <el-table-column label="订单号" width="184" fixed="left">
...
const tableColumns = [
  { key:'projectName',label:'项目名称',minWidth:145 },{ key:'projectTypes',label:'项目类型',minWidth:96 },{ key:'taskDescription',label:'具体任务',minWidth:110 },{ key:'projectStatus',label:'项目状态',width:98 },{ key:'clientShortName',label:'客户简称',width:86,clickHint:'点击客户简称查看关联信息' },{ key:'subClientContact',label:'子客户/联系人',minWidth:125 },{ key:'customerOrderNo',label:'客户单号/项目标识',minWidth:135 },{ key:'languageItemsDisplay',label:'语言方向',minWidth:110 },{ key:'potentialDemand',label:'（潜在）需求量',minWidth:125 },{ key:'customerPriceSummary',label:'客户单价',minWidth:135 },{ key:'assigneeSummary',label:'标注人员安排',minWidth:140 },{ key:'taskDispatchedAt',label:'任务派发时间',width:98,type:'datetime' },{ key:'taskSubmittedAt',label:'任务提交时间',width:98,type:'datetime' },{ key:'clientManagerName',label:'客户经理',width:82 },{ key:'projectPath',label:'项目路径',minWidth:150 },{ key:'quotationPath',label:'报价单路径',minWidth:150 },{ key:'contractPath',label:'合同路径',minWidth:150 },
]
const defaultColumns = ['projectName','projectTypes','projectStatus','clientShortName','languageItemsDisplay','taskDispatchedAt','taskSubmittedAt','clientManagerName']
const { selectedKeys: visibleColumnKeys, isVisible, reset: resetColumns } = useTableColumns('annotation-details-v2',tableColumns,defaultColumns)
