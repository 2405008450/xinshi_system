<template>
  <el-card class="annotation-card compact-list-card">
      ...
      <el-table-column v-for="column in visibleTableColumns" :key="column.key" :prop="column.key" :label="column.label" :width="column.width" :min-width="column.minWidth" :show-overflow-tooltip="!['projectName', 'projectStatus'].includes(column.key)">
        ...
        <template #default="{ row }">
          ...
          <el-dropdown
            ...
            @command="(command) => updatePriority(row, command)"
          >
            <el-tag
              :type="priorityType(row.priority)"
              size="small"
              class="status-switch-tag"
              :class="{ 'is-updating': prioritySavingIds.has(row.id) }"
            >
              <span class="status-switch-text">{{ priorityLabel(row.priority) }}</span>
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item
                  ...
                  :key="item.value"
                  :command="item.value"
                  :disabled="item.value === row.priority || prioritySavingIds.has(row.id)"
                >
                  <span class="status-option-row">
                    <el-tag :type="priorityType(item.value)" size="small" effect="plain">{{ item.label }}</el-tag>
                    <el-icon v-if="item.value === row.priority" class="status-current-icon"><Check /></el-icon>
                  </span>
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
          ...
          <el-select
            ...
            @click.stop
            @change="(value) => updateManager(row, column.key, value)"
          >
            ...
            >
              ...
              <div class="progress-child-content">
                ...
                <div class="progress-child-meta">
                  ...
                  <el-button
                    v-if="canWrite && child.kind === 'progress'"
                    type="danger"
                    link
                    size="small"
    ...
    >
      ...
      <div v-if="progressDeleteTarget" class="progress-delete-preview">
        <div class="progress-delete-preview__meta">{{ formatDateTime(progressDeleteTarget.effectiveOn) }} · {{ progressDeleteTarget.changedByName || '系统' }}</div>
        <div class="progress-delete-preview__note">{{ progressDeleteTarget.changeNote }}</div>
      </div>
      <AppForm ref="progressDeleteFormRef" :model="progressDeleteForm" :rules="progressDeleteRules" label-width="86px" class="progress-delete-form">
        <el-form-item label="删除原因" prop="reason">
          <el-input v-model="progressDeleteForm.reason" type="textarea" :rows="4" maxlength="500" show-word-limit placeholder="请填写删除该条进度的原因，便于后续追溯" />
        </el-form-item>
      </AppForm>
      <template #footer>
    ...
    <DraggableFormDialog v-model="dialogVisible" class="annotation-editor-dialog" width="min(1080px, calc(100vw - 32px))" top="5vh" @closed="onEditorClosed">
      ...
      <div ref="dialogBodyRef" class="editor-body">
        <AppForm ref="formRef" :model="form" :rules="rules" label-width="125px">
          ...
          <section v-if="visibleProjectCustomFields.length" class="form-section">
            <h3>自定义业务字段</h3>
            <el-row :gutter="16"><el-col v-for="field in visibleProjectCustomFields" :key="field.id" :xs="24" :md="12"><el-form-item :label="field.fieldLabel" :required="field.isRequired">
...
const projectRowExtraActions = (row) => [
  ...(canManageArrangementPool && !row.arrangementIncluded
    ? [{ command: 'arrangement-add', label: '加入安排', disabled: arrangementMembershipSavingIds.value.has(row.id) }]
    : []),
  ...(canWrite && row.arrangementIncluded
    ? [{ command: 'arrangement-open', label: '安排' }]
    : []),
  ...projectExtraActions,
  ...(canManageArrangementPool && row.arrangementIncluded
    ? [{ command: 'arrangement-remove', label: '移出安排', disabled: arrangementMembershipSavingIds.value.has(row.id) }]
    : []),
]
const handleProjectExtraAction = (command, row) => {
  if (command === 'arrangement-add') {
    setArrangementMembership(row, true)
    return
  }
  if (command === 'arrangement-open') {
    openArrangement(row)
    return
  }
...
const statusRules={
  ...
  changeNote:[{validator:(_rule,value,callback)=>{const note=String(value||'').trim();if(!note)return callback(new Error(statusEntryMode.value==='progress'?'请填写具体进度':'请填写变更说明'));const maxLength=statusEntryMode.value==='progress'?10000:500;return note.length<=maxLength?callback():callback(new Error(statusEntryMode.value==='progress'?'具体进度不能超过 10000 字':'变更说明不能超过 500 字'))},trigger:['blur','change']}],
}
const progressDeleteVisible=ref(false), progressDeleteSubmitting=ref(false), progressDeleteFormRef=ref(), progressDeleteTarget=ref(null)
const progressDeleteForm=reactive({reason:''})
const progressDeleteRules={
  reason:[{validator:(_rule,value,callback)=>String(value||'').trim()?callback():callback(new Error('请填写删除原因')),trigger:['blur','change']}],
}
...
const progressVisible=ref(false), progressLoading=ref(false), progressRows=ref([]), activeProgressProject=ref(null), progressDialogTab=ref('progress')
...
const annotationFilterFields=computed(()=>[
  ...
  ...visibleProjectCustomFields.value.filter((field)=>field.isActive!==false&&!['image','url'].includes(field.dataType)).map(customFilterDefinition),
])
...
const confirmProgressDelete=async()=>{const target=progressDeleteTarget.value;const project=activeProgressProject.value;if(!target||!project)return;const valid=await progressDeleteFormRef.value?.validate().catch(()=>false);if(!valid)return;progressDeleteSubmitting.value=true;try{const history=await annotationOpsApi.deleteStatusHistoryProgress(target.id,{reason:progressDeleteForm.reason.trim(),expectedUpdatedAt:target.updatedAt||null});progressRows.value=Array.isArray(history)?history:[];statusHistoryCache[project.id]=progressRows.value;progressDeleteVisible.value=false;ElMessage.success('具体进度已删除，操作记录已留存');await fetchData()}catch(error){ElMessage.error(getLocalizedErrorMessage(error,'具体进度删除失败'))}finally{progressDeleteSubmitting.value=false}}
...
.annotation-row-actions{display:inline-grid;grid-template-columns:16px 78px;align-items:center;justify-content:center;gap:6px;width:100px;white-space:nowrap}
.arrangement-status-slot{display:inline-flex;align-items:center;justify-content:center;width:16px;height:24px}
