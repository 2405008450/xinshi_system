<template>
  <el-card class="resource-development-page">
    ...
    <DevelopmentFriendDailyPanel ref="friendPanelRef" :range="filters.range" />
    <div v-loading="loading">
      ...
      <el-collapse v-model="activeDay" accordion @change="changeDay">
        <el-collapse-item v-for="day in days" :key="day.date" :name="day.date"><template #title><strong>{{ chineseDate(day.date) }}</strong><el-tag class="day-count" effect="plain">{{ day.count }} 条开拓记录</el-tag><el-button class="day-count" link type="primary" @click.stop="friendPanelRef.open(day.date)">群聊统计</el-button></template>
